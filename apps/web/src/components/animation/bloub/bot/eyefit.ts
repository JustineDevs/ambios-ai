...
function pire(pts: Point[], emps: Empreinte[], tx: number, ty: number) {
  let marge = Number.POSITIVE_INFINITY;
  let ux = 0;
  let uy = 0;
  for (const e of emps) {
    const x = e.x + tx;
...
function resous(epreuves: Epreuve[]): { x: number; y: number } {
  ...
  // son centre.
  let requis = Number.POSITIVE_INFINITY;
  ...
  let meilleurX = 0;
  let meilleurY = 0;
  let meilleureNorme = Number.POSITIVE_INFINITY;
  ...
  const x = meilleureNorme === Number.POSITIVE_INFINITY ? secoursX : meilleurX;
  const y = meilleureNorme === Number.POSITIVE_INFINITY ? secoursY : meilleurY;
