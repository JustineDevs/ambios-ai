...
export const JsonCodeEditor = ({
  ...
  const contentHeight = useMemo(() => {
    ...
    return Math.max(Number.parseInt(minHeight, 10), lineCount * lineHeight + padding);
  }, [value, placeholder, minHeight]);

  const { height: resizableHeight, resizeHandleProps: internalResizeHandleProps } = useResizable({
    minHeight: 100,
    maxHeight: 1000,
    initialHeight: maxHeight ? Number.parseInt(maxHeight, 10) : 300,
  });
  ...
  const effectiveHeight = useMemo(() => {
    ...
    if (Number.isNaN(parsed)) return contentHeight;
  ...
  return (
    <div className={cn("relative", !noBorder && "rounded-lg border bg-muted/30 shadow-sm")}>
      {overlay && (
        <div
          className={cn(
      ...
      {bottomRightOverlay && (
        <div
          className={cn(
            "absolute z-10 flex items-center gap-1",
            overlayPlacement === "corner" ? "right-2 bottom-2" : "right-1 bottom-1 mr-5",
          )}
        >
          {bottomRightOverlay}
        </div>
      ...
      {showValidation && jsonError && (
        <div className="absolute right-0 bottom-0 left-0 max-h-32 overflow-y-auto overflow-x-hidden bg-destructive/10 p-2 text-destructive text-xs">
          Error: {jsonError}
        </div>
      )}
    </div>
