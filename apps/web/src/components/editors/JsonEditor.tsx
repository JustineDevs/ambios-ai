...
import { cn, formatLabel } from "@/lib/general-utils";
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="min-w-[120px]">
        <DropdownMenuItem onClick={handleDownloadJson} className="cursor-pointer text-xs">
          <FileJson className="mr-2 h-3 w-3" />
          JSON
        </DropdownMenuItem>
        <DropdownMenuItem onClick={handleDownloadCsv} className="cursor-pointer text-xs">
          <FileSpreadsheet className="mr-2 h-3 w-3" />
          CSV
        </DropdownMenuItem>
        {hasFilter && (
          <DropdownMenuItem onClick={handleDownloadFilteredCsv} className="cursor-pointer text-xs">
...
function ViewToggle({
  ...
  return (
    <div className="flex w-fit items-center rounded-md border border-border/50 bg-muted p-0.5">
      <button
        type="button"
        onClick={() => onViewChange("raw")}
        className={cn(
          "flex items-center gap-1 rounded px-1.5 py-0.5 font-medium text-[10px] transition-all",
          view === "raw"
...
function AtomicValue({ value }: { value: unknown }) {
    ...
    return (
      <span className="group inline-flex items-center gap-1">
        <a
          ...
          className="inline-block max-w-[300px] truncate align-bottom text-primary hover:underline"
          title={str}
        >
  ...
  return (
    <span className="group inline-flex items-center gap-1">
      <span
        className={isLong ? "inline-block max-w-[300px] truncate align-bottom" : ""}
...
function ExpandableRow({
  ...
  if (!isExpandable) {
    return (
      <tr className="group border-border/30 border-b hover:bg-muted/30">
        <td className="w-[200px] px-3 py-2 align-top font-medium text-foreground/90">
  ...
  if (count === 0) {
    return (
      <tr className="border-border/30 border-b hover:bg-muted/30">
        <td className="w-[200px] px-3 py-2 align-top font-medium text-foreground/90">
          {displayLabel}
        </td>
  ...
  return (
    <>
      <tr
        className="cursor-pointer border-border/30 border-b hover:bg-muted/30"
      ...
      {expanded && (
        <tr>
          <td colSpan={2} className="p-0">
            <div className="ml-6 border-border/50 border-l">
...
function LazyTableView({
  ...
  if (Array.isArray(data)) {
    if (data.length === 0) {
      return <div className="p-4 text-muted-foreground text-xs">Empty array</div>;
    }
    ...
    if (filteredKey && filteredData.length === 0) {
      return <div className="p-4 text-muted-foreground text-xs">No matches found</div>;
    }
      ...
      <div>
        ...
        {remaining > 0 && (
          <button
            ...
            className="w-full border-border/30 border-t py-2 text-muted-foreground text-xs transition-colors hover:bg-muted/50 hover:text-foreground"
          >
