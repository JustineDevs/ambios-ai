import { isArrowFunction } from "@ambios-ai/shared";
import Editor from "@monaco-editor/react";
import React, { useEffect, useState } from "react";
import { HelpTooltip } from "@/components/utils/HelpTooltip";
import { useMonacoTheme } from "@/hooks/use-monaco-theme";
import { useResizable } from "@/hooks/use-resizable";
import { formatJavaScriptCode } from "@/lib/general-utils";
import { CopyButton } from "../tools/shared/CopyButton";

export const JavaScriptCodeEditor = React.memo(
  ({
    value,
    onChange,
    readOnly = false,
    minHeight = "100px",
    maxHeight = "300px",
    showCopy = true,
    resizable = false,
    isTransformEditor = false,
    autoFormatOnMount = true,
    placeholder,
  }: {
    value: string;
    onChange?: (value: string) => void;
    readOnly?: boolean;
    minHeight?: string;
    maxHeight?: string;
    showCopy?: boolean;
    resizable?: boolean;
    isTransformEditor?: boolean;
    autoFormatOnMount?: boolean;
    placeholder?: string;
  }) => {
    const { theme, onMount } = useMonacoTheme();
    const { height: resizableHeight, resizeHandleProps } = useResizable({
      minHeight: 100,
      maxHeight: 1000,
      initialHeight: Number.parseInt(maxHeight, 10),
    });
    const effectiveHeight = resizable ? resizableHeight : maxHeight;
    const [hasFormatted, setHasFormatted] = useState(false);
    const hasValidPattern = (code: string): boolean => isArrowFunction(code);
    const displayValue = value || "";

    useEffect(() => {
      if (!autoFormatOnMount) return;
      if (!onChange || hasFormatted || !displayValue.trim()) return;
      formatJavaScriptCode(displayValue).then((formatted) => {
        if (formatted !== displayValue) {
          onChange(formatted);
        }
        setHasFormatted(true);
      });
    }, [displayValue, onChange, hasFormatted, autoFormatOnMount]);

    const handleChange = (newValue: string | undefined) => {
      if (!onChange || newValue === undefined) return;
      onChange(newValue);
    };

    return (
      <div className="js-code-editor relative rounded-lg border bg-muted/30 font-mono shadow-sm">
        {(showCopy || isTransformEditor) && (
          <div className="absolute top-1 right-1 z-10 mr-5 flex items-center gap-1">
            {isTransformEditor && (
              <HelpTooltip text="The transform must be an arrow function (sourceData) => { ... } that receives step results and returns the final output. Access each step's data via sourceData.stepId." />
            )}
            {showCopy && <CopyButton text={value || ""} />}
          </div>
        )}
        {resizable && <div {...resizeHandleProps} />}
        {isTransformEditor && displayValue && !hasValidPattern(displayValue) && (
          <div className="flex items-center gap-1 px-3 pt-2 text-[10px] text-amber-600 dark:text-amber-400">
            <span>⚠</span>
            <span>
              Code needs to be a valid arrow function (sourceData) =&gt; {"{"} ... {"}"}
            </span>
          </div>
        )}
        <div className="relative overflow-hidden pr-3" style={{ height: effectiveHeight }}>
          {placeholder && !displayValue && (
            <div className="pointer-events-none absolute top-3 left-14 z-10 whitespace-pre-wrap pl-1 font-mono text-[11px] text-muted-foreground/50">
              {placeholder}
            </div>
          )}
          <Editor
            height={effectiveHeight}
            defaultLanguage="javascript"
            value={displayValue}
            onChange={handleChange}
            onMount={onMount}
            options={{
              readOnly,
              minimap: { enabled: false },
              fontSize: 11,
              lineNumbers: "on",
              glyphMargin: false,
              folding: true,
              scrollBeyondLastLine: false,
              wordWrap: "on",
              contextmenu: false,
              renderLineHighlight: "none",
              scrollbar: {
                vertical: "auto",
                horizontal: "auto",
                verticalScrollbarSize: 8,
                horizontalScrollbarSize: 8,
                alwaysConsumeMouseWheel: false,
              },
              overviewRulerLanes: 0,
              padding: { top: 12, bottom: 12 },
              lineHeight: 18,
              tabSize: 2,
              quickSuggestions: false,
              codeLens: false,
              links: false,
              colorDecorators: false,
              occurrencesHighlight: "off",
              stickyScroll: { enabled: false },
            }}
            theme={theme}
            className="!bg-transparent"
          />
        </div>
      </div>
    );
  },
);
