...
const IntegrationCard: React.FC<{ item: IntegrationItem }> = ({ item }) => {
  return (
    <motion.div
      ...
      className="flex gap-4 border-zinc-100 border-b px-5 py-4 transition-colors last:rounded-b-[14px] last:border-b-[1.6px] hover:bg-zinc-50 dark:border-[#1f1f1f] dark:hover:bg-[#141414]"
    >
      {/* Icon */}
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-2">
          <div className="truncate">
            <h3 className="truncate font-medium text-[14px] text-zinc-900 dark:text-[#EEEEEE]">
              {item.name}
            </h3>
            <p className="mt-0.5 truncate text-[10px] text-zinc-400 uppercase tracking-wider dark:text-[#6b6b6b]">
              {item.entities}
            </p>
          </div>

          {item.available && (
            <span className="shrink-0 rounded-full border border-green-200 bg-green-50 px-2 py-0.5 pt-1 font-bold text-[9px] text-green-600 dark:border-[#1AA420]/70 dark:bg-[#142E17] dark:text-[#1bb022]">
              AVAILABLE
            </span>
          )}
        </div>

        <p className="mt-2 max-w-full text-[12px] text-zinc-500 leading-relaxed dark:text-[#8a8a8a]">
          {item.description}
        </p>
        ...
        <div className="mt-3 flex flex-col justify-between gap-2 sm:flex-row sm:items-center">
          ...
          <span className="whitespace-nowrap text-[10px] text-zinc-400 dark:text-[#6b6b6b]">
...
export const IntegrationsCard: React.FC<IntegrationsCardProps> = ({ items, title }) => {
  ...
  return (
    <div className="flex w-full flex-col items-center bg-transparent px-4 py-8 text-foreground">
      <div className="w-full max-w-145 overflow-hidden rounded-[22px] border border-zinc-200 bg-white shadow-2xl transition-all duration-300 dark:border-[#1f1f1f] dark:bg-[#101010]">
        ...
        <header className="flex items-center justify-between px-5 py-4">
          <h2 className="font-semibold text-sm text-zinc-900 tracking-tight dark:text-white">
            {title}
          </h2>
          <button
            type="button"
        ...
        {/* Filters & Search */}
        <div className="relative flex flex-col items-stretch gap-3 rounded-t-[14px] border-zinc-100 border-t border-b bg-zinc-50 px-5 py-4 md:flex-row md:items-center dark:border-[#1f1f1f] dark:bg-[#171717]">
              ...
              >
                {activePopover === "type" &&
                  typeOptions.map((opt) => (
                    <button
                      ...
                      className="flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-left font-medium text-[12px] text-zinc-700 transition-colors hover:bg-zinc-50 dark:text-[#EEEEEE] dark:hover:bg-white/5"
                    >
                      {opt}
                      {selectedType === opt && (
                ...
                {activePopover === "more" && (
                  <div className="space-y-1 p-2">
                    <p className="px-2 pb-2 font-bold text-[9px] text-zinc-400 uppercase tracking-wider dark:text-[#555]">
                      Advanced
                    </p>
                    <button
                      type="button"
                      className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left text-[11px] text-zinc-600 transition-colors hover:bg-zinc-50 dark:text-[#888] dark:hover:bg-white/5"
                    >
                      Sort by Name
                    </button>
                    <button
                    ...
                    <button
                      type="button"
                      className="flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-left font-medium text-[11px] text-red-500 transition-colors hover:bg-zinc-50 dark:hover:bg-white/5"
                    >
                      Reset Filters
                    </button>
                  </div>
          ...
          <AnimatePresence mode="popLayout">
            ...
            {paginatedItems.length === 0 && (
              ...
              >
                ...
                <p className="font-medium text-[14px] text-zinc-900 tracking-tight dark:text-white">
                ...
                <button
                  ...
                  className="font-bold text-[12px] text-zinc-900 hover:underline dark:text-white"
                >
                  Clear all filters
                </button>
              </motion.div>
        ...
        {/* Footer */}
        <footer className="flex flex-col items-center justify-between gap-4 border-zinc-100 border-t bg-zinc-50/30 px-6 py-4 text-[11px] text-zinc-400 sm:flex-row dark:border-[#1f1f1f] dark:bg-transparent dark:text-[#6b6b6b]">
              ...
              {[...Array(totalPages)].map((_, i) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex h-7 w-7 items-center justify-center rounded-lg font-bold text-[11px] transition-all",
                      currentPage === pageNum
