  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate font-bold text-[#3E3E43] text-[15px] leading-tight sm:text-[17px] dark:text-neutral-200">
          {title}
        </p>
        <p className="truncate text-[#909092] text-[13px] sm:text-[15px] dark:text-neutral-500">
          {desc}
        </p>
      </div>

      <span className="whitespace-nowrap pt-1 text-[#9F9FA1] text-[11px] sm:text-[13px] dark:text-neutral-600">
        {time}
      </span>
    </motion.div>
  );
        ...
        >
          <div className="flex min-w-0 flex-1 items-center gap-3 text-left sm:gap-4">
            ...
            <div className="flex min-w-0 flex-1 flex-col justify-center">
              <motion.p
                layout
                className="truncate font-bold text-[16px] text-neutral-900 tracking-tight sm:text-[17px] dark:text-neutral-100"
              >
                {title}
              </motion.p>
              <AnimatePresence mode="popLayout" initial={false}>
                {!open && (
                  <motion.p
                    ...
                    className="truncate text-[#BFBFC2] text-[14px] tracking-tight sm:text-[15px] dark:text-neutral-500"
                  >
                    {subtitle}
                  </motion.p>
                )}
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              className="border-[#e7e6e6]/60 border-t-2 dark:border-neutral-800"
            >
              <div className="py-2">
                {activities.map((item) => (
                  <ActivityItem key={`${item.title}:${item.desc}:${item.time}`} {...item} />
                ))}
              </div>
            </motion.div>
          )}
