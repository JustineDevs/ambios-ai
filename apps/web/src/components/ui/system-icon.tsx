...
import { Blocks, icons, type LucideIcon } from "lucide-react";
import { memo } from "react";
import { isDarkIconHex, resolveSystemIcon } from "@/lib/general-utils";
  ...
  if (resolved?.type === "simpleicons") {
    return (
      <svg
        ...
        role="img"
        aria-label={resolved.icon.title}
        className={`flex-shrink-0 ${isDarkIconHex(resolved.icon.hex) ? "dark:invert" : ""} ${className}`}
      >
