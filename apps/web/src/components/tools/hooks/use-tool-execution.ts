import { isRequestConfig, isTransformConfig, type Tool, type ToolResult } from "@ambios-ai/shared";
...
import { computeStepOutput, isAbortError, wrapDataSelectorWithLimit } from "@/lib/general-utils";
...
export function useToolExecution(
  ...
  const executeTransform = async (schemaStr: string, transformStr: string): Promise<void> => {
    await executeWithRunId(async () => {
      ...
      const parsedSchema = schemaStr?.trim() ? JSON.parse(schemaStr) : null;
      const client = createClient();
  ...
  const executeTool = async (
    ...
    try {
      ...
      const currentOutputSchema = outputSchema?.trim() ? JSON.parse(outputSchema) : null;
