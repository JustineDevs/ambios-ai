  ...
  const defaultTrigger = (
    <Button variant="glass" size="sm" className="h-8 gap-2 rounded-full">
      ...
      {value && value !== UNCATEGORIZED ? (
        <span className="max-w-[250px] truncate text-xs">{value}</span>
      ) : (
        <span className="max-w-[250px] truncate text-muted-foreground/50 text-xs">No folder</span>
      )}
    </Button>
  );
        ...
        <Command>
          ...
          {showNewInput && (
            <div className="relative border-b px-3 py-2">
              ...
              />
              <CornerDownLeft className="absolute top-1/2 right-5 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
            </div>
          )}
          <CommandList>
            <CommandEmpty>
