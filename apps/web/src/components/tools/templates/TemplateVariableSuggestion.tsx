...
import type { CategorizedSources, CategorizedVariables } from "../context/types";
...
const VariableCommandMenu = forwardRef<VariableCommandMenuRef, VariableCommandMenuProps>(
  (
    ...
    return (
      ...
      >
        {breadcrumbLabel && (
          <div className="truncate border-b px-3 py-1.5 text-muted-foreground text-xs">
            {breadcrumbLabel}
          </div>
        )}
        <div className="overflow-y-auto" style={{ maxHeight: `${MAX_LIST_HEIGHT}px` }}>
          {navState.level === "categories" ? (
            nonEmptyCategories.length === 0 ? (
              <div className="px-3 py-2 text-muted-foreground text-xs">No variables available</div>
            ) : (
              nonEmptyCategories.map((config, index) => (
                <button
                  ...
                  className={cn(
                    "flex w-full items-center justify-between gap-2 whitespace-nowrap px-3 py-1.5 text-left text-sm",
              ...
              return (
                ...
                >
                  <span className="flex items-center gap-1.5 truncate">
                    {typeSymbol && <span className="shrink-0 font-mono text-xs">{typeSymbol}</span>}
        ...
        <button
          ...
          onMouseDown={(e) => e.preventDefault()}
          className={cn(
            "flex w-full items-center gap-2 whitespace-nowrap px-3 py-1.5 text-sm",
