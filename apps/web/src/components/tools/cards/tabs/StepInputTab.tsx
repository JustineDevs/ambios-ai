...
export function StepInputTab({ step, stepIndex, onEdit, isActive = true }: StepInputTabProps) {
  ...
  const computeChipPosition = useCallback((): ChipPosition | null => {
    ...
    const editorHeight = Number.parseInt(height, 10);
  ...
  return (
    <div>
      <div className={cn("relative rounded-lg border bg-muted/30 shadow-sm")} ref={containerRef}>
        {cannotExecuteYet && (
          <div className="pointer-events-none absolute inset-0 z-[5] flex items-center justify-center bg-muted/5 backdrop-blur-[2px]">
            <div className="flex flex-col items-center justify-center py-8 text-muted-foreground">
              <div className="mb-1 text-xs">No data yet</div>
        ...
        <div className={cn("relative cursor-not-allowed overflow-hidden")} style={{ height }}>
          {chipPosition && (
            <div
              className="absolute z-20 flex items-center rounded-sm bg-muted"
              style={{
