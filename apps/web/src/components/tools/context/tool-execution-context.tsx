...
import { useOrgOptional } from "@/app/org-context";
import { buildPreviousStepResults, buildStepInput } from "@/lib/general-utils";
import { buildPaginationData, deriveCurrentItem, extractCredentials } from "@/lib/templating-utils";
import { useToolConfig } from "./tool-config-context";
import {
  type CategorizedSources,
  type CategorizedVariables,
  type DataSelectorResult,
  DEFAULT_STEP_EXECUTION,
  type ExecutionContextValue,
  type StepExecutionState,
  type StepStatus,
  type StepStatusInfo,
  type StepTemplateData,
...
export function ExecutionProvider({ children }: ExecutionProviderProps) {
  ...
  useEffect(() => {
    ...
    prevStepHashesRef.current = currentHashes;
  }, [steps, resetTransformState, hashStepConfig]);
  ...
  }, [steps, stepInputVersion, dataSelectorResults, stepInputs]);
  ...
  const stepTemplateDataMap = useMemo(() => {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      const _allSystemCredentials = flattenAndNamespaceCredentials(systems);
