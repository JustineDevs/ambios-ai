...
import { computeToolPayload } from "@/lib/general-utils";
import { addDraft, getPayload, setPayload } from "@/lib/storage";
import type { PayloadState, ToolConfigContextValue, ToolDefinition } from "./types";
...
export function ToolConfigProvider({
  ...
  const [_baselineVersion, setBaselineVersion] = useState(0); // Triggers re-computation of hasUnsavedChanges
  ...
  useEffect(() => {
    ...
    const loadPayload = async () => {
      try {
        ...
        try {
          const STORAGE_KEY = `ambios-payload:${toolId}`;
