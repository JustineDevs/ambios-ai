...
import { systems as systemTemplates } from "@ambios-ai/shared";
...
export const URLField = forwardRef<URLFieldHandle, URLFieldProps>(function URLField(
  ...
  const getIconForUrl = (url: string): string | null => {
    ...
    try {
      ...
      for (const [_name, system] of Object.entries(systemTemplates)) {
  ...
  const commitUrl = useCallback(
    ...
    [onUrlChange, getIconForUrl, extractQueryParams],
  );
  ...
  }, [initialUrl, getIconForUrl, validateUrl]);
      ...
      <div className="relative flex items-center gap-2">
        <div className="relative flex-1">
          <Input
            ...
            className={cn(
              "h-10 rounded-lg border border-border/50 bg-muted/30 pr-28 shadow-sm transition-colors focus:border-primary/50",
            ...
            <Badge variant="outline">
              {simpleIcon ? (
                <div className="flex items-center">
                  ...
                  >
                    <path
                      className={isDarkIconHex(simpleIcon.hex) ? "dark:invert" : undefined}
                      d={simpleIcon.path}
                    />
                  </svg>
                  <span>
