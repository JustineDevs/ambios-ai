...
import { buildOnboardingRouting, type OnboardingIntentId } from "@/lib/agent/agent-prompts";
import { cn, formatLabel, getAllSimpleIcons } from "@/lib/general-utils";

const ONBOARDING_STORAGE_KEY_PREFIX = "ambios-onboarding-seen";
const ONBOARDING_V1_SEEN_PREFIX = "ambios-onboarding-v1-seen";
...
function SelectionCard({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "w-full rounded-2xl border p-3 text-left backdrop-blur-md transition-all",
        active
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          {subtitle ? <p className="truncate text-muted-foreground text-xs">{subtitle}</p> : null}
        </div>
      </div>
    </button>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="text-center">
        <p className="text-primary/80 text-xs uppercase tracking-[0.2em]">Step 1</p>
        <h3 className="mt-2 font-medium text-2xl text-foreground/95">What is your job title?</h3>
        <p className="mt-2 text-muted-foreground text-sm">
          Help us tailor ambios onboarding to your needs.
        </p>
      </div>
...
function SystemsStep({
  ...
  const renderSystemCard = (
    ...
    <button
      ...
      className={cn(
        "w-full rounded-2xl border p-3 text-left backdrop-blur-sm transition-all",
        isActive
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <p className="truncate font-medium text-sm">{systemOption.label}</p>
      </div>
    </button>
  );
  ...
  return (
    <div className="space-y-5">
      <div className="text-center">
        <p className="text-primary/80 text-xs uppercase tracking-[0.2em]">Step 2</p>
        <h3 className="mt-2 font-medium text-2xl text-foreground/95">What do you work with?</h3>
        <p className="mt-1 text-muted-foreground text-sm">
          Select one or more systems you want ambios to work with.
        </p>
      </div>
      <div className="relative">
        <Search className="pointer-events-none absolute top-3.5 left-3 h-4 w-4 text-muted-foreground/70" />
        <Input
          value={searchTerm}
      ...
      {selectedOptions.length > 0 ? (
        <div className="space-y-2">
          <p className="text-muted-foreground text-xs">Selected systems</p>
      ...
      <div className="grid grid-cols-1 gap-2 pr-1 sm:grid-cols-2 lg:grid-cols-3">
        {newSystemFormExpanded ? (
          ...
          <button
            ...
            className="rounded-2xl border border-border/40 bg-background/70 p-3 text-left backdrop-blur-md transition-all hover:border-border/60 hover:bg-background/80"
          >
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="truncate font-medium text-sm">Custom system</p>
                <p className="truncate text-muted-foreground text-xs">Add any other system</p>
              </div>
            </div>
          </button>
        )}
...
function IntentStep({
  ...
  return (
    <div className="space-y-4">
      <div className="text-center">
        <p className="text-primary/80 text-xs uppercase tracking-[0.2em]">Step 3</p>
        <h3 className="mt-2 font-medium text-2xl text-foreground/95">
          What do you want to do first?
        </h3>
        <p className="mt-1 text-muted-foreground text-sm">
          We'll let the ambios agent know what you want to see.
        </p>
      </div>
      <div className="space-y-2">
...
export function OnboardingModal() {
  ...
  const assistantState =
    currentStep === 0 ? "connecting" : currentStep === 1 ? "searching" : "approval";
  ...
  return (
    <Dialog open={isOpen}>
      <DialogContent
        className="top-0 left-0 z-[70] h-screen w-screen max-w-none translate-x-0 translate-y-0 gap-0 overflow-hidden border-none bg-transparent p-0 shadow-none sm:rounded-none"
      ...
      >
        <DialogHeader className="sr-only">
          <DialogTitle>Welcome to ambios</DialogTitle>
          ...
          <div className="relative flex h-full max-h-[92vh] w-full max-w-6xl flex-col rounded-[28px] border border-white/30 bg-background/60 p-6 shadow-2xl backdrop-blur-xl sm:p-8">
            <div className="mb-5 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                />
                <div>
                  <p className="font-medium text-foreground/90 text-sm">Welcome to ambios</p>
                  <p className="text-muted-foreground text-xs">Personalize your experience</p>
                </div>
              </div>
            </div>
