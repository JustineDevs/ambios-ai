import type { ExecutionFileEnvelope, Tool, ToolCall } from "@ambios-ai/shared";
import {
  type Dispatch,
  type SetStateAction,
  useCallback,
...
import { resolveFileInputBindings, validateFileReferences } from "@/lib/agent/agent-helpers";
import { abortExecution, generateUUID, shouldDebounceAbort } from "@/lib/client-utils";
...
export function useToolExecution({
  ...
  const executeToolConfig = useCallback(
    ...
    [createClient, filePayloads, tool.input?.payload, editablePayload, tool.input?.files],
  );
