...
import type { MessagePart, ToolCall } from "@ambios-ai/shared";
import {
  Blocks,
  BookOpen,
  ChevronRight,
...
function ShimmerText({ text, className }: { text: string; className?: string }) {
  return (
    <span
      className={cn(
        "inline-block animate-shimmer-text bg-[length:200%_100%] bg-clip-text text-transparent",
...
export function BackgroundToolIndicator({ tool, count }: BackgroundToolIndicatorProps) {
  ...
  return (
    <div
      className={cn(
        "inline-flex items-center gap-1.5 py-0.5 text-xs",
        isActive ? "text-muted-foreground" : "text-muted-foreground/60",
      )}
    >
      <Icon className={cn("h-3 w-3", isActive && "animate-shimmer-icon")} />
      {isActive ? (
      ...
      {showCollapsible ? (
        <div className="py-0.5">
          <button
            ...
            className="inline-flex cursor-pointer select-none items-center gap-1 text-muted-foreground/60 text-xs transition-colors hover:text-muted-foreground/80"
          >
            <ChevronRight
