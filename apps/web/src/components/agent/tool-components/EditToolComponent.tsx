...
import { useToolConfigOptional } from "@/components/tools/context/tool-config-context";
import { Button } from "@/components/ui/button";
import { ErrorMessage } from "@/components/ui/error-message";
import { FileChip } from "@/components/ui/file-chip";
import { findDraftInMessages } from "@/lib/agent/agent-context";
import {
  createToolInteractionEntry,
  type ToolMutation,
} from "@/lib/agent/agent-tools/tool-call-state";
import type { EditToolSaveResult } from "@/lib/agent/agent-types";
import { applyDiffsToConfig, enrichDiffsWithTargets } from "@/lib/config-diff-utils";
import { deleteAllDrafts } from "@/lib/storage";
...
export function EditToolComponent({
  ...
  const handleDiffApprovalComplete = useCallback(
    async (result: {
      ...
      let effectiveSaveResult: EditToolSaveResult;
  ...
  const handleRunWithApprovedDiffs = useCallback(
    async (approvedDiffs: ToolDiff[], overridePayload?: Record<string, any>) => {
      ...
      const _toolId = currentConfig?.id || originalConfig?.id;
          ...
          <div className="space-y-3">
            {inputFileChips.length > 0 && (
              <div className="space-y-1.5">
                <div className="font-medium text-muted-foreground text-xs">File Inputs</div>
              ...
              return (
                <>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-sm">Tool Edited Successfully</span>
