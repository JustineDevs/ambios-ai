...
import type { ExecutionFileEnvelope, Message, MessageReference } from "@ambios-ai/shared";
import { useCallback, useRef } from "react";
import type { AgentRequest, ToolExecutionPolicies } from "@/lib/agent/agent-types";
...
export function useAgentRequest({
  ...
  const sendAgentRequest = useCallback(
    async (
      ...
      try {
        const response = await requestOperation("agentChat", {
          method: "POST",
          headers: {
