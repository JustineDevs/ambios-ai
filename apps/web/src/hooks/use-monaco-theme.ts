...
function defineThemes(monaco: typeof Monaco | any) {
  ...
  monaco.editor.defineTheme("ambios-light", {
    base: "vs",
    inherit: true,
    rules: [
  ...
  monaco.editor.defineTheme("ambios-dark", {
    base: "vs-dark",
    inherit: true,
    rules: [
      { token: "comment", foreground: "6A737D", fontStyle: "italic" },
...
function updateAllEditorsTheme(theme: "light" | "dark") {
  const themeName = theme === "dark" ? "ambios-dark" : "ambios-light";
...
export function useMonacoTheme() {
  ...
  useEffect(() => {
    ...
    loader.init().then((monaco) => {
      ...
      monaco.editor.setTheme(theme === "dark" ? "ambios-dark" : "ambios-light");
    });
    return () => {
      mounted = false;
    };
  ...
  const themeName = currentTheme === "dark" ? "ambios-dark" : "ambios-light";
