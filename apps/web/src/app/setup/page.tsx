        ...
        details: [
          {
            ...
            value: "Managed through your secure connection",
            status: "ok",
          },
...
export default function SetupPage(): ReactElement {
  ...
  const objectStorageConfigured =
    fileProvider === "aws"
      ? Boolean(awsBucket && awsKey && awsSecret)
      : fileProvider === "minio"
  ...
  return (
    <LockedPage>
      <div className="min-w-0 overflow-auto p-4 sm:p-6 md:p-8">
        <div className="mx-auto min-w-0 max-w-5xl space-y-6">
          <EnterprisePage
            eyebrow="Runtime configuration"
            title="Prepare your workspace"
            description="Review capability readiness and configuration blockers before governed work begins. Values are read from this server and sensitive values remain masked."
          >
            <EnterpriseSummary
              items={[
                {
                  label: "Workspace readiness",
                  value: readinessBlocked ? "Needs setup" : "Ready",
                  detail: readinessBlocked
                    ? "Resolve the required blockers below"
                    : "Core runtime prerequisites are configured",
                  tone: readinessBlocked ? "warning" : "success",
                },
                {
                  label: "Persistence",
                  value: postgresConfigured ? "Available" : "Blocked",
                  detail: postgresConfigured
                    ? "Postgres is configured"
                    : "Database credentials required",
                  tone: postgresConfigured ? "success" : "danger",
                },
                {
                  label: "Model capability",
                  value: llmProvider.model ? "Available" : "Blocked",
                  detail: llmProvider.model
                    ? `${llmProvider.label} is selected`
                    : "Select a supported provider",
                  tone: llmProvider.model ? "success" : "danger",
                },
                {
                  label: "Object storage",
                  value: objectStorageConfigured ? "Available" : "Optional",
                  detail: objectStorageConfigured
                    ? "Storage credentials are present"
                    : "Required only for file features",
                },
              ]}
            />
            <EnterpriseState
              ...
              title={
                readinessBlocked
                  ? "Configuration blockers require operator attention"
                  : "Readiness checks passed"
              }
              description={
                readinessBlocked
                  ? "Read-only inspection remains available. Resolve the marked required values before relying on governed actions."
                  : "The configured runtime can expose its available capabilities; provider execution still follows connector and approval state."
              }
            />
          </EnterprisePage>
          ...
          <section>
            <Card className={GLASS_CARD}>
              <CardHeader className="pb-2">
                ...
                <CardDescription>
                  PostgreSQL connection used for all persistent data.
                </CardDescription>
              </CardHeader>
