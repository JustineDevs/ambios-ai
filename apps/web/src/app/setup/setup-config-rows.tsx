...
import { cn, isDarkIconHex } from "@/lib/general-utils";
    ...
    >
      ...
      <div className="mt-1.5 flex min-h-[2rem] min-w-0 max-w-full items-center gap-1 overflow-hidden rounded-md border border-border/25 bg-muted/15 pr-1">
        <div className="min-w-0 flex-1 overflow-x-auto overscroll-x-contain [-ms-overflow-style:none] [scrollbar-width:thin] [&::-webkit-scrollbar]:h-1.5">
          <code className="inline-block min-h-[1.25rem] whitespace-nowrap px-2 py-1 text-foreground/90 text-xs leading-snug">
            {value}
          </code>
        </div>
        {showCopy ? (
          <div className="flex shrink-0 items-center self-stretch border-border/30 border-l pl-0.5">
...
function BrandIcon({ providerKey }: { providerKey: ProviderKey }) {
  ...
  if (!brand) {
    return (
      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-border/35 bg-background/25 font-medium text-muted-foreground text-xs backdrop-blur-md">
        {meta.monogram}
      </div>
    );
  }
  return (
    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-border/35 bg-background/25 backdrop-blur-md">
      <svg viewBox="0 0 24 24" className="h-[18px] w-[18px]" role="img" aria-label={brand.title}>
        <path
          className={isDarkIconHex(brand.hex) ? "dark:invert" : undefined}
          fill={`#${brand.hex}`}
          d={brand.path}
        />
      </svg>
  ...
  return (
    <div className="min-w-0 max-w-full rounded-2xl border border-border/35 bg-background/20 p-4 backdrop-blur-md">
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0 flex-1 pt-1 font-medium text-sm">
          {summary.label}
