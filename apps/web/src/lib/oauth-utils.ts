...
import { AmbiOSClient, operationPath, resolveOAuthCertAndKey, systems } from "@ambios-ai/shared";
...
const executeClientCredentialsFlow = async (params: {
  ...
  const callbackUrl = `${window.location.origin}/oauth/callback?grant_type=client_credentials&state=${encodeURIComponent(btoa(JSON.stringify(state)))}`;

  const makeRequest = async () => {
    try {
      await fetch(operationPath("initIntegrationOAuth"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ apiKey, state: btoa(JSON.stringify(state)) }),
...
const executeAuthorizationCodeFlow = (params: {
  ...
  const initAndOpenPopup = async () => {
    ...
    await fetch(operationPath("initIntegrationOAuth"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ apiKey, codeVerifier, state: btoa(JSON.stringify(state)) }),
      credentials: "same-origin",
    });
...
export const triggerOAuthFlow = (
  ...
  if (!usingTemplate && oauthFields.client_secret && oauthFields.client_id && apiKey && endpoint) {
    ...
    const client = new AmbiOSClient({ apiKey, apiEndpoint: apiEndpoint || endpoint });
