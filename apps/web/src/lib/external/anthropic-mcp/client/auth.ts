...
import type { FetchLike } from "../shared/transport";
...
export async function startAuthorization(
  ...
  if (metadata) {
    ...
    if (!metadata.code_challenge_methods_supported?.includes(codeChallengeMethod)) {
      throw new Error(
...
export async function refreshAuthorization(
  ...
  return OAuthTokensSchema.parse({ refresh_token: refreshToken, ...responseBody });
}
