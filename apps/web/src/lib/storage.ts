...
let dbPromise: Promise<IDBPDatabase<AmbiOSDB>> | null = null;

function getDB(): Promise<IDBPDatabase<AmbiOSDB>> {
  if (!dbPromise) {
    dbPromise = openDB<AmbiOSDB>(DB_NAME, DB_VERSION, {
      upgrade(db) {
...
async function migrateFromLocalStorage(): Promise<void> {
  ...
  migrationPromise = (async () => {
    ...
    try {
      // Migrate payloads
      const payloadKeys = Object.keys(localStorage).filter((k) => k.startsWith("ambios-payload:"));
      for (const key of payloadKeys) {
        const toolId = key.replace("ambios-payload:", "");
      ...
      const cacheKeys = Object.keys(localStorage).filter(
        (k) => k.includes("cache") || (k.startsWith("ambios-") && !k.startsWith("ambios-payload:")),
      );
...
function notifyDraftChange(toolId: string) {
  draftChangeListeners.forEach((fn) => {
    fn(toolId);
  });
}
