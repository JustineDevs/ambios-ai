...
import { SKILL_GATED_TOOLS, TOOL_REGISTRY } from "./registries/tool-registry";
...
export function validateAgentRequest(body: any): ValidatedAgentRequest {
  ...
  if (body.model !== undefined && (typeof body.model !== "string" || body.model.length > 100)) {
    throw new Error("model must be a valid model identifier");
  }
...
export function registerSkillGatedTools(
  ...
  for (const skill of loadedSkills) {
    ...
    if (gated) for (const toolName of gated) gatedToolNames.add(toolName);
  }
