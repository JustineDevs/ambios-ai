...
import { decodeBase64ToUint8Array } from "@/lib/file-utils";
import { findDraftInMessages } from "./agent-context";
import type { EditToolSaveResult, ToolExecutionContext } from "./agent-types";
...
export async function tryTriggerScrapeJob(
  ...
  try {
    await ctx.ambiosClient.triggerSystemDocumentationScrapeJob(systemId, scrapeInput);
    return null;
  } catch (error: any) {
...
export const resolveOriginalConfig = async (
  ...
  if (toolId) {
    try {
      return await ctx.ambiosClient.getWorkflow(toolId);
    } catch {}
  }
...
export const formatSystemKnowledgeForOutput = (template: SystemConfig) => {
  ...
  const snakeCaseOauth: Record<string, any> = {};
