...
ambios is an integration platform that builds and runs deterministic multi-step workflows ("tools") connecting APIs, databases, file servers and other systems. ambios agents generate tool configurations during building — execution is deterministic JavaScript, no LLMs involved.
...
- Tight observability and error handling in your app stack and on the ambios web app

### 2) Scheduler (web interface)

Documentation: https://docs.ambios.cloud/enterprise/scheduling
...
- ambios triggers execution at the specified interval/timezone
...
5. Set up alerting/monitoring on failure rates and duration. ambios has a dedicated notifications feature for this. You can find it under Control Panel -> Notifications. Enterprise-only.

### 3) MCP deployment (for agent interfaces)

Documentation: https://docs.ambios.cloud/mcp/using-the-mcp

Use this when LLM agents (Cursor, Claude Desktop, Langdock, custom MCP clients, etc.) should discover and run ambios tools as callable functions.

**How ambios MCP works**

- The ambios MCP is designed to provide an interface for an LLM to execute your pre-built ambios tools
- Agent clients connect once, discover available tool definitions, then invoke them with structured inputs
- ambios executes the underlying deterministic workflow and returns normalized output to the agent runtime

**Setup model**

1. Run/connect to the ambios MCP server endpoint
2. Configure client authentication by providing your sg API key and setting up the correct endpoint (e.g. "https://mcp.ambios.ai" on hosted) in MCP client config
...
"env": {
"AUTH_HEADER": "Bearer {YOUR_AMBIOS+API_KEY}"
}
}
}
}
...
Install via: npm install -g @ambios-ai/cli
Documentation: https://docs.ambios.cloud/getting-started/cli-skills
...
- The CLI authenticates to ambios via your API key and inherits your RBAC access rules
...
ambios CLI — build, run, and manage integration tools

Options:
-V, --version output the version number
--api-key <key> ambios API key
--endpoint <url> ambios API endpoint
--json force JSON output
-h, --help display help for command

Commands:
init [options] Set up ambios CLI configuration
update [options] Update the ambios CLI to the latest version
skill [topic] Print the ambios skill reference (SKILL.md) for AI agents
tool Manage ambios tools
system Manage ambios systems
