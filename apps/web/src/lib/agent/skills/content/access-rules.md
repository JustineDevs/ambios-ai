...
On ambios, roles define tool-level and system-level allowlists. Users can have multiple roles. Roles and access rules are only available on enterprise organizations. Non-enterprise orgs do not have RBAC — all users have admin access on personal organizations.
...
// Only allow GET requests to a registry-approved read operation.
stepConfig.method === "GET" && stepConfig.operationId?.startsWith("get");
