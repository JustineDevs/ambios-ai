...
import { findTemplateForSystem, systems } from "@ambios-ai/shared/templates";
import * as jsonpatch from "fast-json-patch";
import { applyDiffsToConfig } from "@/lib/config-diff-utils";
import { resolveOAuthConfig } from "@/lib/oauth-utils";
...
import { SKILL_GATED_TOOLS } from "./tool-sets";
...
const buildToolDefinition = (): ToolDefinition => ({
  name: "build_tool",
  description: `Builds a new ambios tool by accepting the full tool configuration JSON.
...
const runRunTool = async (input: any, ctx: ToolExecutionContext) => {
  ...
  if (toolId) {
    try {
      toolConfig = await ctx.ambiosClient.getWorkflow(toolId);
    } catch (error: any) {
      return {
        success: false,
        error: error.message,
  ...
  try {
    const result: ToolResult = isDraft
      ? await ctx.ambiosClient.runToolConfig({
          tool: toolConfig,
          payload: resolvedPayload,
          files: executionFiles,
          traceId,
        })
      : await ctx.ambiosClient.runTool({
          toolId: toolId!,
          payload: resolvedPayload,
          files: executionFiles,
...
const runEditTool = async (input: any, ctx: ToolExecutionContext) => {
  ...
  if (toolId) {
    if (shouldUseCurrentPlaygroundDraft) {
      ...
      try {
        const savedTool = await ctx.ambiosClient.getWorkflow(toolId);
        if (!savedTool) {
          return {
            success: false,
...
const processEditToolConfirmation = async (
  ...
  if (parsedOutput.confirmationState === "partial") {
...
const runSaveTool = async (input: any, ctx: ToolExecutionContext) => {
  ...
  try {
    ...
    const savedTool = await ctx.ambiosClient.upsertWorkflow(toolId, toolToSave);

    const apiEndpoint = ctx.ambiosClient.apiEndpoint;
    return {
      success: true,
      toolId: savedTool.id,
      persistence: "saved",
      webhookUrl: `${apiEndpoint}/mcp`,
    };
  } catch (error: any) {
    return {
      success: false,
...
const runCreateSystem = async (input: any, ctx: ToolExecutionContext) => {
  ...
  try {
    const result = await ctx.ambiosClient.createSystem(createPayload);

    if (fileUploadResult.files.length > 0) {
      try {
        await ctx.ambiosClient.uploadSystemFileReferences(result.id, fileUploadResult.files);
    ...
    if (documentationUrl) {
      try {
        await ctx.ambiosClient.triggerSystemDocumentationScrapeJob(result.id, {
    ...
    if (openApiUrl) {
      try {
        await ctx.ambiosClient.fetchOpenApiSpec(result.id, openApiUrl);
      } catch {}
    }
...
const processCreateSystemConfirmation = async (
  ...
  if (parsedOutput.confirmationState === "confirmed") {
    ...
    try {
      const result = await ctx.ambiosClient.createSystem({
        ...cleanSystemConfig,
...
const runEditSystem = async (input: any, ctx: ToolExecutionContext) => {
  const { sensitiveCredentials, files, scrapeUrl, scrapeKeywords, environment, ...systemInput } =
    input;
  ...
  if (env === "prod" && systemInput.credentials) {
    return {
      success: false,
      error: "Production connector credentials must be managed through the secure connection flow.",
    };
  }
  ...
  try {
    const existingSystem = await ctx.ambiosClient.getSystem(patchPayload.id, {
      environment: env,
    });
    if (!existingSystem) {
      return {
    ...
    if (!hasPatch && hasFiles) {
      await ctx.ambiosClient.uploadSystemFileReferences(patchPayload.id, fileUploadResult.files);
      const updated = await ctx.ambiosClient.getSystem(patchPayload.id, { environment: env });
    ...
    const result = await ctx.ambiosClient.updateSystem(patchPayload.id, patchPayload, {
      environment: env,
    });
...
const processEditSystemConfirmation = async (
  ...
  if (parsedOutput.confirmationState === "confirmed") {
    ...
    try {
      const existingSystem = await ctx.ambiosClient.getSystem(cleanSystemConfig.id, {
        environment: env,
      });
      if (!existingSystem) {
        return {
      ...
      if (!hasPatch && hasFilesToUpload && !("error" in fileUploadResult)) {
        await ctx.ambiosClient.uploadSystemFileReferences(
          cleanSystemConfig.id,
          fileUploadResult.files,
        );
        const updated = await ctx.ambiosClient.getSystem(cleanSystemConfig.id, {
          environment: env,
        });
      ...
      const result = await ctx.ambiosClient.updateSystem(
        cleanSystemConfig.id,
        {
          ...cleanSystemConfig,
          credentials: finalCredentials,
...
const runCallSystem = async (
  ...
  try {
    ...
    const result = await ctx.ambiosClient.executeStep({
      step,
      payload: {},
...
const runSearchDocumentation = async (input: any, ctx: ToolExecutionContext) => {
  ...
  try {
    const result = await ctx.ambiosClient.searchSystemDocumentation(systemId, keywords);
...
const runAuthenticateOAuth = async (input: any, ctx: ToolExecutionContext) => {
  ...
  try {
    const system = await ctx.ambiosClient.getSystem(input.systemId, { environment: env });
    if (!system) {
      return {
        success: false,
...
const processAuthenticateOAuthConfirmation = async (
  ...
  if (parsedOutput.confirmationState === "oauth_success") {
    ...
    if (env === "prod") {
      return {
        output: JSON.stringify({
          success: false,
          error:
            "Production connector credentials must be managed through the secure connection flow.",
        }),
        status: "completed",
      };
    }
    ...
    try {
      const currentSystem = await ctx.ambiosClient.getSystem(systemId, { environment: env });
      ...
      await ctx.ambiosClient.updateSystem(
        systemId,
...
const runGetRuns = async (
  ...
  try {
    ...
    const result = await ctx.ambiosClient.listRuns({
      toolId,
      limit: fetchLimit,
      page,
    ...
    const runs = fetchResults
      ? await Promise.all(
          pageItems.map(async (run: any) => {
            try {
              const full = await ctx.ambiosClient.getRun(run.runId);
              if (full) {
                return {
                  ...stripRun(run),
                  data: full.data,
...
const runFindTool = async (
  ...
  try {
    if (input.id) {
      const tool = await ctx.ambiosClient.getWorkflow(input.id);
      if (!tool) {
...
const runFindSystem = async (
  ...
  try {
    const { items: allSystems } = await ctx.ambiosClient.listSystems(1000, 1);
...
const loadSkillDefinition = (): ToolDefinition => ({
  name: "load_skill",
  description: `Loads ambios skills into context.
...
const _findRoleDefinition = (): ToolDefinition => ({
  name: "find_role",
  description:
