import { AmbiOSClient, type Message } from "@ambios-ai/shared";
...
export class AgentClient {
  ...
  private ambiosClient: AmbiOSClient;

  constructor(config: AgentClientConfig) {
    this.config = config;

    this.model = config.modelInstance ?? initializeAIModel({ modelOverride: config.model });

    this.ambiosClient = new AmbiOSClient({
      apiKey: config.token,
  ...
  async *streamResponse(validated: ValidatedAgentRequest): AsyncGenerator<StreamChunk> {
    ...
    const executionContext: ToolExecutionContext = {
      ...
      ambiosClient: this.ambiosClient,
