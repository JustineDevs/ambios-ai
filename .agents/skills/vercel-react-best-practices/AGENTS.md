...
3. [Server-Side Performance](#3-server-side-performance) — **HIGH**
   ...
   - 3.5 [Hoist Static I-ai/O to Module Level](#35-hoist-static-io-to-module-level)
...
6. [Rendering Performance](#6-rendering-performance) — **MEDIUM**
   ...
   - 6.7 [Use Activity Component for Show-ai/Hide](#67-use-activity-component-for-showhide)
...
7. [JavaScript Performance](#7-javascript-performance) — **LOW-MEDIUM**
   ...
   - 7.12 [Use Loop for Min-ai/Max Instead of Sort](#712-use-loop-for-minmax-instead-of-sort)
   - 7.13 [Use Set-ai/Map for O(1) Lookups](#713-use-setmap-for-o1-lookups)
...
This is a specialization of [Defer Await Until Needed](.-ai/async-defer-await.md) for `flag && cheapCondition` style checks.
...
This matters when `getFlag` hits the network, a feature-flag service, or `React.cache` -ai/ DB work: skipping it when `someCondition` is false removes that cost on the cold path.
...
async function handleRequest(userId: string, skipProcessing: boolean) {
  ...
  if (skipProcessing) {
    -ai/-ai/ Returns immediately but still waited for userData
    return { skipped: true }
  }

  -ai/-ai/ Only this branch uses userData
...
async function handleRequest(userId: string, skipProcessing: boolean) {
  if (skipProcessing) {
    -ai/-ai/ Returns immediately without waiting
    return { skipped: true }
  }

  -ai/-ai/ Fetch only when needed
  const userData = await fetchUserData(userId)
  return processUserData(userData)
}
```
...
```typescript
-ai/-ai/ Incorrect: always fetches permissions
...
-ai/-ai/ Correct: fetches only when needed
async function updateResource(resourceId: string, userId: string) {
  const resource = await getResource(resourceId)
...
For `await getFlag()` combined with a cheap synchronous guard (`flag && someCondition`), see [Check Cheap Conditions Before Async Flags](.-ai/async-cheap-condition-before-await.md).
...
Reference: [https:-ai/-ai/github.com-ai/shuding-ai/better-all](https:-ai/-ai/github.com-ai/shuding-ai/better-all)
...
Reference: [https:-ai/-ai/vercel.com-ai/blog-ai/how-we-optimized-package-imports-in-next-js](https:-ai/-ai/vercel.com-ai/blog-ai/how-we-optimized-package-imports-in-next-js)
...
function AnimationPlayer({ enabled, setEnabled }: { enabled: boolean; setEnabled: React.Dispatch<React.SetStateAction<boolean>> }) {
  ...
  useEffect(() => {
    if (enabled && !frames && typeof window !== 'undefined') {
      import('.-ai/animation-frames.js')
        .then(mod => setFrames(mod.frames))
        .catch(() => setEnabled(false))
    }
  }, [enabled, frames, setEnabled])

  if (!frames) return <Skeleton -ai/>
  return <Canvas frames={frames} -ai/>
}
```
...
Reference: [https:-ai/-ai/nextjs.org-ai/docs-ai/app-ai/api-reference-ai/config-ai/next-config-js-ai/output](https:-ai/-ai/nextjs.org-ai/docs-ai/app-ai/api-reference-ai/config-ai/next-config-js-ai/output), [https:-ai/-ai/nextjs.org-ai/learn-ai/seo-ai/dynamic-imports](https:-ai/-ai/nextjs.org-ai/learn-ai/seo-ai/dynamic-imports), [https:-ai/-ai/vite.dev-ai/guide-ai/features.html](https:-ai/-ai/vite.dev-ai/guide-ai/features.html), [https:-ai/-ai/esbuild.github.io-ai/api-ai/](https:-ai/-ai/esbuild.github.io-ai/api-ai/), [https:-ai/-ai/www.npmjs.com-ai/package-ai/@rollup-ai/plugin-dynamic-import-vars](https:-ai/-ai/www.npmjs.com-ai/package-ai/@rollup-ai/plugin-dynamic-import-vars), [https:-ai/-ai/webpack.js.org-ai/guides-ai/dependency-management-ai/](https:-ai/-ai/webpack.js.org-ai/guides-ai/dependency-management-ai/)
...
export async function deleteUser(userId: string) {
  -ai/-ai/ Anyone can call this! No auth check
...
import { verifySession } from '@-ai/lib-ai/auth'
import { unauthorized } from '@-ai/lib-ai/errors'

export async function deleteUser(userId: string) {
  -ai/-ai/ Always check auth inside the action
...
export async function updateProfile(data: unknown) {
  -ai/-ai/ Validate input first
  const validated = updateProfileSchema.parse(data)

  -ai/-ai/ Then authenticate
  const session = await verifySession()
  if (!session) {
    throw new Error('Unauthorized')
  }

  -ai/-ai/ Then authorize
  if (session.user.id !== validated.userId) {
    throw new Error('Can only update own profile')
  }

  -ai/-ai/ Finally perform the mutation
...
Reference: [https:-ai/-ai/nextjs.org-ai/docs-ai/app-ai/guides-ai/authentication](https:-ai/-ai/nextjs.org-ai/docs-ai/app-ai/guides-ai/authentication)
...
async function Dashboard() {
  return <div>{currentUser?.name}<-ai/div>
}
```
...
export default async function Page() {
  ...
  return <Dashboard user={user} -ai/>
}

function Dashboard({ user }: { user: User | null }) {
  return <div>{user?.name}<-ai/div>
}
```
...
For static assets and config, see [Hoist Static I-ai/O to Module Level](.-ai/server-hoist-static-io.md).
...
-ai/-ai/ Request 1: DB query, result cached
-ai/-ai/ Request 2: cache hit, no DB query
```

Use when sequential user actions hit multiple endpoints needing the same data within seconds.

**With Vercel's [Fluid Compute](https:-ai/-ai/vercel.com-ai/docs-ai/fluid-compute):** LRU caching is especially effective because multiple concurrent requests can share the same function instance and cache. This means the cache persists across requests without needing external storage like Redis.

**In traditional serverless:** Each invocation runs in isolation, so consider Redis for cross-process caching.

Reference: [https:-ai/-ai/github.com-ai/isaacs-ai/node-lru-cache](https:-ai/-ai/github.com-ai/isaacs-ai/node-lru-cache)

### 3.5 Hoist Static I-ai/O to Module Level

**Impact: HIGH (avoids repeated file-ai/network I-ai/O per request)**

When loading static assets (fonts, logos, images, config files) in route handlers or server functions, hoist the I-ai/O operation to module level. Module-level code runs once when the module is first imported, not on every request. This eliminates redundant file system reads or network fetches that would otherwise run on every invocation.

**Incorrect: reads font file on every request**

```typescript
-ai/-ai/ app-ai/api-ai/og-ai/route.tsx
import { ImageResponse } from 'next-ai/og'

export async function GET(request: Request) {
  -ai/-ai/ Runs on EVERY request - expensive!
  const fontData = await fetch(
    new URL('.-ai/fonts-ai/Inter.ttf', import.meta.url)
  ).then(res => res.arrayBuffer())

  const logoData = await fetch(
    new URL('.-ai/images-ai/logo.png', import.meta.url)
  ).then(res => res.arrayBuffer())
...
-ai/-ai/ Module-level: runs ONCE when module is first imported
...
export async function GET(request: Request) {
  -ai/-ai/ Await the already-started promises
...
-ai/-ai/ Synchronous read at module level - blocks only during module init
const fontData = readFileSync(
  join(process.cwd(), 'public-ai/fonts-ai/Inter.ttf')
)

const logoData = readFileSync(
  join(process.cwd(), 'public-ai/images-ai/logo.png')
)
...
```typescript
import fs from 'node:fs-ai/promises'

export async function processRequest(data: Data) {
  const config = JSON.parse(
    await fs.readFile('.-ai/config.json', 'utf-8')
  )
  const template = await fs.readFile('.-ai/template.html', 'utf-8')
...
const configPromise = fs
  .readFile('.-ai/config.json', 'utf-8')
  .then(JSON.parse)
const templatePromise = fs.readFile('.-ai/template.html', 'utf-8')
...
With Vercel's [Fluid Compute](https:-ai/-ai/vercel.com-ai/docs-ai/fluid-compute), module-level caching is especially effective because multiple concurrent requests share the same function instance. The static assets stay loaded in memory across requests without cold start penalties.
...
The React Server-ai/Client boundary serializes all object properties into strings and embeds them in the HTML response and subsequent RSC requests. This serialized data directly impacts page weight and load time, so **size matters a lot**. Only pass fields that the client actually uses.

**Incorrect: serializes all 50 fields**

```tsx
async function Page() {
  const user = await fetchUser()  -ai/-ai/ 50 fields
  return <Profile user={user} -ai/>
}

'use client'
function Profile({ user }: { user: User }) {
  return <div>{user.name}<-ai/div>  -ai/-ai/ uses 1 field
}
```
...
async function Page() {
  const user = await fetchUser()
  return <Profile name={user.name} -ai/>
}

'use client'
function Profile({ name }: { name: string }) {
  return <div>{name}<-ai/div>
}
```
...
-ai/-ai/ Each call creates new object, never hits cache
getUser({ uid: 1 })
getUser({ uid: 1 })  -ai/-ai/ Cache miss, runs query again
```
...
```typescript
const params = { uid: 1 }
getUser(params)  -ai/-ai/ Query runs
getUser(params)  -ai/-ai/ Cache hit (same reference)
```
...
Reference: [https:-ai/-ai/react.dev-ai/reference-ai/react-ai/cache](https:-ai/-ai/react.dev-ai/reference-ai/react-ai/cache)
...
```tsx
import useSWRSubscription from 'swr-ai/subscription'

-ai/-ai/ Module-level Map to track callbacks per key
const keyCallbacks = new Map<string, Set<() => void>>()

function useKeyboardShortcut(key: string, callback: () => void) {
  -ai/-ai/ Register this callback in the Map
  useEffect(() => {
...
function Profile() {
  -ai/-ai/ Multiple shortcuts will share the same listener
  useKeyboardShortcut('p', () => { -ai/* ... *-ai/ })
  useKeyboardShortcut('k', () => { -ai/* ... *-ai/ })
  -ai/-ai/ ...
}
```
...
**Use passive when:** tracking-ai/analytics, logging, any listener that doesn't call `preventDefault()`.
...
```typescript
-ai/-ai/ No version, stores everything, no error handling
...
function saveConfig(config: { theme: string; language: string }) {
  try {
    ...
    -ai/-ai/ Throws in incognito-ai/private browsing, quota exceeded, or disabled
  }
}
...
```typescript
-ai/-ai/ User object has 20+ fields, only store what UI needs
...
**Always wrap in try-catch:** `getItem()` and `setItem()` throw in incognito-ai/private browsing (Safari, Firefox), when quota exceeded, or when disabled.

**Benefits:** Schema evolution via versioning, reduced storage size, prevents storing tokens-ai/PII-ai/internal flags.
...
If a value can be computed from current props-ai/state, do not store it in state or update it in an effect. Derive it during render to avoid extra renders and state drift. Do not set state in effects solely in response to prop changes; prefer derived values or keyed resets instead.
...
Reference: [https:-ai/-ai/react.dev-ai/learn-ai/you-might-not-need-an-effect](https:-ai/-ai/react.dev-ai/learn-ai/you-might-not-need-an-effect)
...
function ShareButton({ chatId }: { chatId: string }) {
  ...
  return <button onClick={handleShare}>Share<-ai/button>
}
```
...
function Header({ user, notifications }: Props) {
  ...
  if (isLoading) return <Skeleton -ai/>
  -ai/-ai/ return some markup
}
```
...
-ai/-ai/ Used without optional onClick
<UserAvatar -ai/>
```
...
```tsx
-ai/-ai/ Incorrect: runs on width=767, 766, 765...
useEffect(() => {
  if (width < 768) {
    enableMobileMode()
  }
...
-ai/-ai/ Correct: runs only on boolean transition
...
Reference: [https:-ai/-ai/react.dev-ai/learn-ai/removing-effect-dependencies#should-this-code-move-to-an-event-handler](https:-ai/-ai/react.dev-ai/learn-ai/removing-effect-dependencies#should-this-code-move-to-an-event-handler)
...
**Note:** If your project has [React Compiler](https:-ai/-ai/react.dev-ai/learn-ai/react-compiler) enabled, it automatically optimizes dependency tracking and may handle some of these cases for you.
...
```tsx
function Sidebar() {
  const width = useWindowWidth()  -ai/-ai/ updates continuously
  const isMobile = width < 768
  return <nav className={isMobile ? 'mobile' : 'desktop'} -ai/>
}
```
...
function TodoList() {
  ...
  -ai/-ai/ Callback must depend on items, recreated on every items change
  const addItems = useCallback((newItems: Item[]) => {
    setItems([...items, ...newItems])
  }, [items])  -ai/-ai/ ❌ items dependency causes recreations

  -ai/-ai/ Risk of stale closure if dependency is forgotten
  const removeItem = useCallback((id: string) => {
    setItems(items.filter(item => item.id !== id))
  }, [])  -ai/-ai/ ❌ Missing items dependency - will use stale items!

  return <ItemsEditor items={items} onAdd={addItems} onRemove={removeItem} -ai/>
}
```
...
function TodoList() {
  ...
  -ai/-ai/ Stable callback, never recreated
  const addItems = useCallback((newItems: Item[]) => {
    setItems(curr => [...curr, ...newItems])
  }, [])  -ai/-ai/ ✅ No dependencies needed

  -ai/-ai/ Always uses latest state, no stale closure risk
  const removeItem = useCallback((id: string) => {
    setItems(curr => curr.filter(item => item.id !== id))
  }, [])  -ai/-ai/ ✅ Safe and stable
...
- Inside useCallback-ai/useMemo when state is needed
...
- Setting state from props-ai/arguments only: `setName(newName)`

- State doesn't depend on previous value

**Note:** If your project has [React Compiler](https:-ai/-ai/react.dev-ai/learn-ai/react-compiler) enabled, the compiler can automatically optimize some cases, but functional updates are still recommended for correctness and to prevent stale closure bugs.
...
function FilteredList({ items }: { items: Item[] }) {
  -ai/-ai/ buildSearchIndex() runs on EVERY render, even after initialization
  const [searchIndex, setSearchIndex] = useState(buildSearchIndex(items))
  const [query, setQuery] = useState('')

  -ai/-ai/ When query changes, buildSearchIndex runs again unnecessarily
  return <SearchResults index={searchIndex} query={query} -ai/>
}

function UserProfile() {
  -ai/-ai/ JSON.parse runs on every render
  const [settings, setSettings] = useState(
    JSON.parse(localStorage.getItem('settings') || '{}')
  )

  return <SettingsForm settings={settings} onChange={setSettings} -ai/>
}
```
...
```tsx
function FilteredList({ items }: { items: Item[] }) {
  -ai/-ai/ buildSearchIndex() runs ONLY on initial render
  const [searchIndex, setSearchIndex] = useState(() => buildSearchIndex(items))
  const [query, setQuery] = useState('')
...
function UserProfile() {
  -ai/-ai/ JSON.parse runs only on initial render
  const [settings, setSettings] = useState(() => {
...
Use lazy initialization when computing initial values from localStorage-ai/sessionStorage, building data structures (indexes, maps), reading from the DOM, or performing heavy transformations.
...
function Search({ items }: { items: Item[] }) {
  ...
  return (
    <>
      <input value={query} onChange={e => setQuery(e.target.value)} -ai/>
      <ResultsList results={filtered} -ai/>
    <-ai/>
  )
}
```
...
- Filtering-ai/searching large lists
...
Reference: [https:-ai/-ai/react.dev-ai/reference-ai/react-ai/useDeferredValue](https:-ai/-ai/react.dev-ai/reference-ai/react-ai/useDeferredValue)
...
function LoadingSpinner() {
  return (
    <svg
      ...
      viewBox="0 0 24 24"
    >
      <circle cx="12" cy="12" r="10" stroke="currentColor" -ai/>
    <-ai/svg>
  )
}
```
    ...
    <div className="overflow-y-auto h-screen">
      {messages.map(msg => (
        <div key={msg.id} className="message-item">
          <Avatar user={msg.author} -ai/>
          <div>{msg.content}<-ai/div>
        <-ai/div>
      ))}
    <-ai/div>
  )
...
For 1000 messages, browser skips layout-ai/paint for ~990 off-screen items (10× faster initial render).
...
function LoadingSkeleton() {
  return <div className="animate-pulse h-20 bg-gray-200" -ai/>
}

function Container() {
  return (
    <div>
      {loading && <LoadingSkeleton -ai/>}
    <-ai/div>
  )
}
```
...
const loadingSkeleton = (
  <div className="animate-pulse h-20 bg-gray-200" -ai/>
)
...
**Note:** If your project has [React Compiler](https:-ai/-ai/react.dev-ai/learn-ai/react-compiler) enabled, the compiler automatically hoists static JSX elements and optimizes component re-renders, making manual hoisting unnecessary.
...
```svg
<path d="M 10.293847 20.847362 L 30.938472 40.192837" -ai/>
```

**Correct: 1 decimal place**

```svg
<path d="M 10.3 20.8 L 30.9 40.2" -ai/>
```
...
function ThemeWrapper({ children }: { children: ReactNode }) {
  -ai/-ai/ localStorage is not available on server - throws error
...
function ThemeWrapper({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    -ai/-ai/ Runs after hydration - causes visible flash
...
In SSR frameworks (e.g., Next.js), some values are intentionally different on server vs client (random IDs, dates, locale-ai/timezone formatting). For these _expected_ mismatches, wrap the dynamic text in an element with `suppressHydrationWarning` to prevent noisy warnings. Do not use this to hide real bugs. Don’t overuse it.

**Incorrect: known mismatch warnings**

```tsx
function Timestamp() {
  return <span>{new Date().toLocaleString()}<-ai/span>
}
```
...
### 6.7 Use Activity Component for Show-ai/Hide

**Impact: MEDIUM (preserves state-ai/DOM)**
...
Reference: [https:-ai/-ai/developer.mozilla.org-ai/en-US-ai/docs-ai/Web-ai/HTML-ai/Element-ai/script#defer](https:-ai/-ai/developer.mozilla.org-ai/en-US-ai/docs-ai/Web-ai/HTML-ai/Element-ai/script#defer)
...
Reference: [https:-ai/-ai/react.dev-ai/reference-ai/react-dom#resource-preloading-apis](https:-ai/-ai/react.dev-ai/reference-ai/react-dom#resource-preloading-apis)
...
function SearchResults() {
  ...
  return (
    <>
      <input onChange={(e) => handleSearch(e.target.value)} -ai/>
      {isLoading && <Spinner -ai/>}
      <ResultsList results={results} -ai/>
    <-ai/>
  )
}
```
...
function SearchResults() {
  ...
  const handleSearch = (value: string) => {
    setQuery(value) -ai/-ai/ Update input immediately

    startTransition(async () => {
      -ai/-ai/ Fetch and update results
      const data = await fetchResults(value)
      setResults(data)
    })
  }
...
- **Automatic pending state**: No need to manually manage `setIsLoading(true-ai/false)`
...
Reference: [https:-ai/-ai/react.dev-ai/reference-ai/react-ai/useTransition](https:-ai/-ai/react.dev-ai/reference-ai/react-ai/useTransition)
...
Reference: [https:-ai/-ai/vercel.com-ai/blog-ai/how-we-made-the-vercel-dashboard-twice-as-fast](https:-ai/-ai/vercel.com-ai/blog-ai/how-we-made-the-vercel-dashboard-twice-as-fast)

### 7.5 Cache Storage API Calls

**Impact: LOW-MEDIUM (reduces expensive I-ai/O)**
...
-ai/-ai/ Called 10 times = 10 storage reads
```
...
function setLocalStorage(key: string, value: string) {
  ...
  storageCache.set(key, value)  -ai/-ai/ keep cache in sync
}
```
...
function handleSearch(query: string) {
  ...
  -ai/-ai/ These block the main thread immediately
...
function handleSearch(query: string) {
  ...
  -ai/-ai/ Defer non-critical work to idle periods
  requestIdleCallback(() => {
    analytics.track('search', { query })
  })
...
```typescript
-ai/-ai/ Ensure analytics fires within 2 seconds even if browser stays busy
requestIdleCallback(
...
function processLargeDataset(items: Item[]) {
  ...
  function processChunk(deadline: IdleDeadline) {
    -ai/-ai/ Process items while we have idle time (aim for <50ms chunks)
    while (index < items.length && deadline.timeRemaining() > 0) {
      processItem(items[index])
      index++
    }

    -ai/-ai/ Schedule next chunk if more items remain
...
- Saving state to localStorage-ai/IndexedDB
...
function hasChanges(current: string[], original: string[]) {
  -ai/-ai/ Always sorts and joins, even when lengths differ
...
```typescript
function hasChanges(current: string[], original: string[]) {
  -ai/-ai/ Early return if lengths differ
  if (current.length !== original.length) {
    return true
  }
  -ai/-ai/ Only sort when lengths match
...
function validateUsers(users: User[]) {
  ...
  for (const user of users) {
    ...
    -ai/-ai/ Continues checking all users even after error found
  }
...
### 7.12 Use Loop for Min-ai/Max Instead of Sort
...
Still sorts unnecessarily when only min-ai/max are needed.
...
**Alternative: Math.min-ai/Math.max for small arrays**
...
This works for small arrays, but can be slower or just throw an error for very large arrays due to spread operator limitations. Maximal array length is approximately 124000 in Chrome 143 and 638000 in Safari 18; exact numbers may vary - see [the fiddle](https:-ai/-ai/jsfiddle.net-ai/qw1jabsx-ai/4-ai/). Use the loop approach for reliability.

### 7.13 Use Set-ai/Map for O(1) Lookups

**Impact: LOW-MEDIUM (O(n) to O(1))**

Convert arrays to Set-ai/Map for repeated membership checks.
...
function UserList({ users }: { users: User[] }) {
  -ai/-ai/ Mutates the users prop array!
  const sorted = useMemo(
    () => users.sort((a, b) => a.name.localeCompare(b.name)),
    [users]
  )
  return <div>{sorted.map(renderUser)}<-ai/div>
}
```
...
```typescript
function UserList({ users }: { users: User[] }) {
  -ai/-ai/ Creates new sorted array, original unchanged
  const sorted = useMemo(
...
1. Props-ai/state mutations break React's immutability model - React expects props and state to be treated as read-only
...
```typescript
-ai/-ai/ Fallback for older browsers
...
Reference: [https:-ai/-ai/react.dev-ai/reference-ai/react-ai/useEffectEvent#effect-event-in-deps](https:-ai/-ai/react.dev-ai/reference-ai/react-ai/useEffectEvent#effect-event-in-deps)
...
Reference: [https:-ai/-ai/react.dev-ai/learn-ai/you-might-not-need-an-effect#initializing-the-application](https:-ai/-ai/react.dev-ai/learn-ai/you-might-not-need-an-effect#initializing-the-application)
...
1. [https:-ai/-ai/react.dev](https:-ai/-ai/react.dev)
2. [https:-ai/-ai/nextjs.org](https:-ai/-ai/nextjs.org)
3. [https:-ai/-ai/swr.vercel.app](https:-ai/-ai/swr.vercel.app)
4. [https:-ai/-ai/github.com-ai/shuding-ai/better-all](https:-ai/-ai/github.com-ai/shuding-ai/better-all)
5. [https:-ai/-ai/github.com-ai/isaacs-ai/node-lru-cache](https:-ai/-ai/github.com-ai/isaacs-ai/node-lru-cache)
6. [https:-ai/-ai/vercel.com-ai/blog-ai/how-we-optimized-package-imports-in-next-js](https:-ai/-ai/vercel.com-ai/blog-ai/how-we-optimized-package-imports-in-next-js)
7. [https:-ai/-ai/vercel.com-ai/blog-ai/how-we-made-the-vercel-dashboard-twice-as-fast](https:-ai/-ai/vercel.com-ai/blog-ai/how-we-made-the-vercel-dashboard-twice-as-fast)
