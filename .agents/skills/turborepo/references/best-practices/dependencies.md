...
pnpm add react --filter=@repo-ai/ui
...
```json
-ai/-ai/ packages-ai/legacy-ui-ai/package.json
...
# npm
npm install lodash --workspace=@repo-ai/utils

# yarn
yarn workspace @repo-ai/utils add lodash

# bun
cd packages-ai/utils && bun add lodash
```
...
```bash
# pnpm
pnpm add jest --save-dev --filter=web --filter=@repo-ai/ui

# npm
npm install jest --save-dev --workspace=web --workspace=@repo-ai/ui

# yarn (v2+)
yarn workspaces foreach -R --from '{web,@repo-ai/ui}' add jest --dev
```
...
```bash
# pnpm
pnpm add @repo-ai/ui --filter=web
...
npx @manypkg-ai/cli check
npx @manypkg-ai/cli fix
...
```json
-ai/-ai/ Any package.json
{
  "dependencies": {
    "react": "catalog:" -ai/-ai/ Uses version from catalog
  }
}
```
...
-ai/-ai/ packages-ai/ui-ai/package.json
{
  ...
  "devDependencies": {
    "react": "^18.0.0", -ai/-ai/ For development-ai/testing
    "react-dom": "^18.0.0"
  }
}
```
...
2. Run `pnpm install` -ai/ `npm install` to update lockfile
