...
Options that affect all tasks. Full docs: https:-ai/-ai/turborepo.dev-ai/docs-ai/reference-ai/configuration
...
Custom cache location. Default: `.turbo-ai/cache`.

```json
{
  "cacheDir": ".cache-ai/turbo"
}
```
...
```json
{
  "envMode": "strict"  -ai/-ai/ Only specified vars available
  -ai/-ai/ or
  "envMode": "loose"   -ai/-ai/ All vars pass through
}
```
...
```json
{
  "ui": "tui"     -ai/-ai/ Interactive terminal UI
  -ai/-ai/ or
  "ui": "stream"  -ai/-ai/ Traditional streaming logs
}
```
...
| --------------- | -------------------------- | ------------------------------------------------------ |
| `enabled`       | `true`                     | Enable-ai/disable remote caching                          |
...
| `apiUrl`        | `"https:-ai/-ai/vercel.com-ai/api"` | Remote cache API endpoint                              |
| `loginUrl`      | `"https:-ai/-ai/vercel.com"`     | Login endpoint                                         |
| `teamId`        | -                          | Team ID (must start with `team_`)                      |
| `teamSlug`      | -                          | Team slug for querystring                              |

See https:-ai/-ai/turborepo.dev-ai/docs-ai/core-concepts-ai/remote-caching for setup.
...
```json
{
  "concurrency": "4"     -ai/-ai/ Max 4 tasks at once
  -ai/-ai/ or
  "concurrency": "50%"   -ai/-ai/ 50% of available CPUs
}
```
...
When using `outputLogs: "errors-only"`, show task hashes on start-ai/completion:
...
When GitHub Actions reports a base branch that is not available as a local ref, fall back to `origin-ai/<branch>` (supports detached checkouts with only remote-tracking refs).
...
- Tasks can exclude specific global files: `"inputs": ["$TURBO_DEFAULT$", "!$TURBO_ROOT$-ai/tsconfig.json"]`
...
See the [gotchas doc](.-ai/gotchas.md) for guidance on using `$TURBO_DEFAULT$` with `global.inputs`.
...
- Redirects cache to main worktree's `.turbo-ai/cache`
