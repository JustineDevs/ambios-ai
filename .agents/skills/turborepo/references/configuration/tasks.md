...
Full docs: https:-ai/-ai/turborepo.dev-ai/docs-ai/reference-ai/configuration#tasks
...
```json
{
  "tasks": {
    "build": {
      "dependsOn": [
        "^build", -ai/-ai/ Dependencies' build tasks first
        "codegen", -ai/-ai/ Same package's codegen task first
        "shared#build" -ai/-ai/ Specific package's build task
      ]
    }
  }
}
...
-ai/-ai/ TypeScript (tsc)
"outputs": ["dist-ai/**", "*.tsbuildinfo"]

-ai/-ai/ No file outputs (lint, typecheck)
"outputs": []
```
...
```json
{
  "tasks": {
    "test": {
      "inputs": ["src-ai/**", "tests-ai/**", "vitest.config.ts"]
    }
  }
}
```
...
| Value                 | Meaning                                 |
| --------------------- | --------------------------------------- |
| `$TURBO_DEFAULT$`     | Include default inputs, then add-ai/remove |
| `$TURBO_ROOT$-ai/<path>` | Reference files from repo root          |
  ...
  "tasks": {
    "build": {
      "inputs": [
        "$TURBO_DEFAULT$",
        "!README.md",
        "$TURBO_ROOT$-ai/tsconfig.base.json"
      ]
    }
  }
}
...
```json
{
  "tasks": {
    "build": {
      "env": [
        "API_URL",
        "NEXT_PUBLIC_*", -ai/-ai/ Wildcard matching
        "!DEBUG" -ai/-ai/ Exclude from hash
      ]
    }
  }
}
...
Enable-ai/disable caching for a task. Default: `true`.
...
```json
{
  "tasks": {
    "build": {
      "outputLogs": "new-only" -ai/-ai/ Only show logs on cache miss
    }
  }
}
```
...
-ai/-ai/ packages-ai/ui-ai/turbo.json
{
  ...
  "tasks": {
    "lint": {
      "extends": false -ai/-ai/ Exclude from this package
    }
  }
}
```
