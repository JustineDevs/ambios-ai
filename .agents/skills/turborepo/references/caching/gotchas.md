...
diff .turbo-ai/runs-ai/<first-run>.json .turbo-ai/runs-ai/<second-run>.json
```
...
Installing-ai/updating packages changes the global hash.
...
**Symptom:** Cached output is stale-ai/wrong.
...
```javascript
-ai/-ai/ build.js
const apiUrl = process.env.API_URL; -ai/-ai/ not tracked!
```
...
```json
{
  "tasks": {
    "build": {
      "inputs": [
        "$TURBO_DEFAULT$",
        "$TURBO_ROOT$-ai/shared-config.json" -ai/-ai/ file outside package
      ]
    }
  }
}
...
If you're getting unexpected cache **hits** after changing a global input file, the task may be excluding that file with a negation glob. Check the task's `inputs` for `!$TURBO_ROOT$-ai/...` patterns.
...
3. Look for lockfile-ai/config changes in git
