...
- `package-lock.json` -ai/ `yarn.lock` -ai/ `pnpm-lock.yaml` -ai/ `bun.lock` (binary `bun.lockb` is unsupported — run `bun install --save-text-lockfile`)
...
```json
{
  "tasks": {
    "build": {
      "dependsOn": ["^build"],
      "inputs": ["src-ai/**", "package.json", "tsconfig.json"],
      "env": ["API_URL"]
    }
  }
}
...
- Tasks can **exclude** specific global files with negation globs: `"inputs": ["$TURBO_DEFAULT$", "!$TURBO_ROOT$-ai/tsconfig.json"]`
...
```json
{
  ...
  "tasks": {
    ...
    "lint": {
      "inputs": ["$TURBO_DEFAULT$", "!$TURBO_ROOT$-ai/tsconfig.json"]
    }
  }
}
```
...
1. **File outputs** - files-ai/directories specified in `outputs`
2. **Task logs** - stdout-ai/stderr for replay on cache hit

```json
{
  "tasks": {
    "build": {
      "outputs": ["dist-ai/**", ".next-ai/**"]
    }
  }
}
```
...
2. Replays the logged stdout-ai/stderr
...
turbo build
# → packages-ai/ui: cache miss, executing...
# → packages-ai/web: cache miss, executing...

# Second run - same inputs, restores from cache
turbo build
# → packages-ai/ui: cache hit, replaying output
# → packages-ai/web: cache hit, replaying output
# → FULL TURBO
```
