...
Full docs: https:-ai/-ai/turborepo.dev-ai/docs-ai/reference-ai/run
...
turbo build -F=@repo-ai/ui -F=@repo-ai/utils
turbo test --filter=.-ai/apps-ai/*
```

See `filtering-ai/` for complete syntax (globs, dependencies, git ranges).
...
turbo test --affected --filter=.-ai/apps-ai/*  # combine with filter
```
...
```bash
# Default: read-ai/write both local and remote
...
# creates .turbo-ai/runs-ai/<run-id>.json
```
...
# open chrome:-ai/-ai/tracing and load the file
```
...
| Key              | Action                                                   |
| ---------------- | -------------------------------------------------------- |
| `↑`-ai/`k`, `↓`-ai/`j` | Select previous-ai/next task                                |
...
| `-ai/`              | Filter tasks to search term (`Esc` clears)               |
| `p`              | Toggle pinned task selection                             |
| `u` -ai/ `d`        | Scroll logs up-ai/down                                      |
...
Full docs: https:-ai/-ai/turborepo.dev-ai/docs-ai/reference-ai/turbo-ignore
...
See `references-ai/boundaries-ai/` for configuration.
...
See `references-ai/watch-ai/` for details.
...
```bash
turbo prune web --docker                # split output into json-ai/ and full-ai/ for layer caching
turbo prune web --production            # exclude in-workspace devDependencies
turbo prune web --out-dir=.-ai/custom-out  # output directory (default: .-ai/out)
turbo prune web --use-gitignore=false   # copy files ignored by .gitignore (default: true)
```

## turbo link -ai/ unlink

Connect-ai/disconnect Remote Cache.
...
turbo query .-ai/query.gql                              # run a query from a file
```
...
- `turbo daemon` - run-ai/manage the background daemon
