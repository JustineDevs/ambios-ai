...
```bash
# Run build-ai/test-ai/lint only in changed packages and their dependents
...
If you change `@repo-ai/ui`, packages that import `@repo-ai/ui` (like `apps-ai/web`) need to re-run their tasks to verify they still work with the changes.

### Customizing --affected

```bash
# Use a different base branch
TURBO_SCM_BASE=origin-ai/develop turbo run build --affected
...
turbo run build --filter=...[origin-ai/main]
...
```bash
--filter=.-ai/apps-ai/*           # all packages in apps-ai/
--filter=.-ai/packages-ai/ui      # specific directory
```

### By Dependencies-ai/Dependents
...
--filter=.-ai/apps-ai/* --filter=!admin   # apps except admin
```
...
| Only changed (no dependents)       | `turbo run build --filter=[origin-ai/main]`                   |
| Changed + dependencies             | `turbo run build --filter=[origin-ai/main]...`                |
