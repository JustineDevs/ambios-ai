...
turbo run test --filter=@acme-ai/api
```
...
```bash
turbo run build --filter=.-ai/apps-ai/*
```

Run in specific directories:

```bash
turbo run build --filter=.-ai/apps-ai/web --filter=.-ai/apps-ai/api
```
...
```bash
turbo run build --filter=@acme-ai/*
```
...
```bash
turbo run build --filter=.-ai/apps-ai/* --filter=!admin
```
...
```bash
turbo run build --filter={.-ai/apps-ai/*}[HEAD^1]
```
...
```bash
turbo run deploy --filter={.-ai/apps-ai/*}[main...HEAD]
```
