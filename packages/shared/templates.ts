...
import type { ConnectionFieldDef } from "./utils";
...
2. Set application type to "Web application" and add your AmbiOS instance callback URL as an authorized redirect URI (for OSS: https://<your-ambios-host>/oauth/callback)
...
5. AmbiOS auto-refreshes tokens - no manual token management needed
...
2. Set redirect URI to your AmbiOS instance callback URL (https://<your-ambios-host>/oauth/callback)
...
`,
  ...
  microsoft: {
    ...
    systemSpecificInstructions: `Azure App Registration Required: Create an app in Azure Portal → App registrations with redirect URI: your AmbiOS instance callback URL (for OSS: https://<your-ambios-host>/oauth/callback)
...
- PAGINATION IN AMBIOS: Use cursorBased pagination with cursorPath "@odata.nextLink":
...
PAGINATION: Uses @odata.nextLink (a full URL for the next page). In ambios, use offsetBased pagination with $top and $skip since BC supports $skip:
...
`,
  ...
  ambiosEmail: {
    name: "ambiosEmail",
    apiUrl: "https://api.ambios.cloud/v1/notify/email",
    regex: "^.*api\\.ambios\\.cloud.*$",
    icon: "lucide:mail",
    docsUrl: "https://docs.ambios.cloud/guides/email-service",
    preferredAuthType: "apikey",
    keywords: ["email", "send", "notification", "transactional", "demo", "ambios", "api key"],
  },
  googleAds: {
    name: "googleAds",
...
AMBIOS EXAMPLE - JWT generation in a transform step (Direct Trust):
\`\`\`javascript
(data) => {
