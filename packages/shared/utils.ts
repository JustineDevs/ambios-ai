...
export async function generateUniqueId({
  ...
  if (match) {
    root = match[1];
    counter = Number.parseInt(match[2], 10) + 1;
  }
...
export function sampleResultObject(value: any, sampleSize = 10, seen = new WeakSet()): any {
  ...
  return Object.fromEntries(
    Object.entries(value).map(([key, val]) => [key, sampleResultObject(val, sampleSize, seen)]),
  );
}
 ...
 */
export function truncateForLLM(value: unknown, maxChars = 5000, sampleSize = 10): string {
...
export function composeUrl(host: string, path: string) {
  ...
  if (!/^(https?|postgres(ql)?|ftp(s)?|sftp|smb|file):\/\//i.test(normalizedHost)) {
    normalizedHost = `https://${normalizedHost}`;
  }

  // Trim slashes in one pass
  const cleanHost = normalizedHost.endsWith("/") ? normalizedHost.slice(0, -1) : normalizedHost;
...
export function truncateRunResult(
  ...
  const truncateSampleableFields = (value: any, depth = 0): any => {
