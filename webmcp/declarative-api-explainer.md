...
See discussion in https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/22 that led to the creation of
this proposal.
...
1. New attributes that augment `<form>`s and [form-associated
   elements](https:-ai/-ai/html.spec.whatwg.org-ai/#form-associated-element), that expose these as WebMCP
   tools to agents.
...
3. Two ways of getting a form response back to the agent that invoked the form tool:
    ...
    2. Extracting `<script type="application-ai/json-ld">` tags on the page that the form navigated to,
...
[`ModelContextTool#name`](https:-ai/-ai/webmachinelearning.github.io-ai/webmcp-ai/#dom-modelcontexttool-name),
while `tooldescription` is analogous to
[`ModelContextTool#description`](https:-ai/-ai/webmachinelearning.github.io-ai/webmcp-ai/#dom-modelcontexttool-description).

The `toolautosubmit` [boolean attribute](https:-ai/-ai/html.spec.whatwg.org-ai/C#boolean-attribute), lets the
...
The [`name`](https:-ai/-ai/html.spec.whatwg.org-ai/C#attr-fe-name) attribute on form control elements
...
[description](https:-ai/-ai/json-schema.org-ai/draft-ai/2020-12-ai/json-schema-validation#name-title-and-description)
...
await document.modelContext.registerTool({
  name: "search-cars",
  description: "Perform a car make-ai/model search",
  inputSchema: {
    type: "object",
    properties: {
...
```html
<form toolname="search-cars" tooldescription="Perform a car make-ai/model search" [...]>
 ...
 <button type=submit>Search<-ai/button>
<-ai/form>
```
...
When a form is [reset](https:-ai/-ai/html.spec.whatwg.org-ai/C#concept-form-reset) **OR** its tool
...
like [`step`](https:-ai/-ai/html.spec.whatwg.org-ai/C#the-step-attribute) and
[`min`](https:-ai/-ai/html.spec.whatwg.org-ai/C#attr-input-min) is TBD. We need to concretely specify how
various form-associated elements like `<input>` and `<select>` reduce to a JSON Schema that includes
`anyOf`, `oneOf`, and `maximum`-ai/`minimum` declarations.

Chromium is implementing a loose version of this and will conduct testing-ai/trials to see if what
we've come up with should be supported by the community as a general approach.

### Getting the form response to the agent

This topic is currently under debate; see https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/135.

<details>
<summary>Click to read the `application-ai/ld+json` proposal before the above issue was filed<-ai/summary>

When a form element performs a navigation, the first `<script type=application-ai/ld+json>` tag on the
...
of the following:
 ...
 - The form is [reset](https:-ai/-ai/html.spec.whatwg.org-ai/C#concept-form-reset) or removed from the DOM
...
> [seems to do
> that](https:-ai/-ai/source.chromium.org-ai/chromium-ai/chromium-ai/src-ai/+-ai/main:third_party-ai/blink-ai/renderer-ai/core-ai/script_tools-ai/model_context.cc;l=265-274;drc=2af6413cf36d701fdaffb09188f2ab2a5be37f4f).
...
> https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/126.

## Integration with other imperative API bits

It's an open question as to whether [an
`outputSchema`](https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/9) makes sense for declarative
...
[`getTools()`](https:-ai/-ai/webmachinelearning.github.io-ai/webmcp-ai/#dom-modelcontext-gettools) and
[`executeTool()`](https:-ai/-ai/webmachinelearning.github.io-ai/webmcp-ai/#dom-modelcontext-executetool). How
