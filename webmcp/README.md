...
TypeScript type definitions for WebMCP are available in the [`webmcp-types`](https:-ai/-ai/www.npmjs.com-ai/package-ai/webmcp-types) npm package.
...
- **UI Disintermediation & Context Loss**: Backend integrations take place directly between the agent and the service, bypassing the service's web UI -ai/ browser experience.
...
```mermaid
graph TD
    subgraph WB["<b><i>Web browser<-ai/i><-ai/b>"]
    ...
    AI["<b><i>AI agent platform<-ai/i><-ai/b>"]
    TP["<b><i>Third-party service<br>(example.com)<-ai/i><-ai/b>"]
...
graph TD
    ...
    subgraph TP["<b><i>Third-party service (example.com)<-ai/i><-ai/b>"]
        MCP[("MCP Server")]
    end
...
Web pages that use WebMCP can be thought of as in-page [Model Context Protocol (MCP)](https:-ai/-ai/modelcontextprotocol.io-ai/introduction) servers that implement tools exposing client-side logic and DOM interaction rather than server-side APIs. WebMCP enables collaborative workflows where users and agents work together within the same web interface, leveraging existing application logic while maintaining shared context and user control.
...
Jen wants to create a yard sale flyer on `https:-ai/-ai/easely.example`. She wants to filter templates and make visual edits. Instead of navigating menus, she interacts with her browser's agent:
...
Maya is shopping for dresses on `http:-ai/-ai/wildebloom.example-ai/shop`.
...
John is a software developer performing a code review in [Gerrit](https:-ai/-ai/www.gerritcodereview.com-ai/). The interface is complex, but the page registers helpful tools to inspect trybot statuses and retrieve logs, perfect for agents that are typically trained on everyday usage, and may otherwise do a poor job actuating such complicated interfaces.
...
await document.modelContext.registerTool({
  ...
  async execute({ text }) {
    -ai/-ai/ Reuse existing client-side application logic and update UI.
...
-ai/-ai/ To unregister the tool later, abort the signal.
-ai/-ai/ controller.abort();
```
...
For forms and standard HTML inputs, a declarative counterpart to the imperative API allows the browser to automatically synthesize tool definitions from `<form>` elements. This is detailed in the [Declarative API Explainer](.-ai/declarative-api-explainer.md). It will be soon folded into this explainer document.
...
By default, WebMCP is enabled in top-level `Window`s and its same-origin iframes, but access can be delegated to cross-origin iframes using the [Permissions Policy](https:-ai/-ai/developer.mozilla.org-ai/en-US-ai/docs-ai/Web-ai/HTTP-ai/Guides-ai/Permissions_Policy) `allow="tools"`:

  ```html
  <iframe src="https:-ai/-ai/chat-bot-provider.example-ai/" allow="tools"><-ai/iframe>
  ```

Calls to `document.modelContext.registerTool()` will return a promise rejected with `NotAllowedError` DOMException when the permission is disabled, whether by the `allow` attribute or the `Permissions-Policy: tools=()` header. Handling of declarative tool registration errors, including when the permission is disabled is TBD; see [Issue #182](https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/182).

#### Cross-origin iframe exposure: `registerTool() and `exposedTo`

By default, tools registered by a document are only exposed to itself, same-origin documents in the same tree, and built-in browser agents (see this <a href=#built-in-agent-default-exposure>discussion<-ai/a>). To support author-provided agents running in frames, developers can selectively share tools with specific secure origins via the `exposedTo` option:
...
}, { exposedTo: ["https:-ai/-ai/trusted-partner.example"] });
```
...
```js
-ai/-ai/ Discover tools exposed by same-origin frames in the tree (default)
...
-ai/-ai/ Discover additional tools provided by a cross-origin frame (in addition to
-ai/-ai/ same-origin ones):
const crossOriginTools = await document.modelContext.getTools({
  fromOrigins: ["https:-ai/-ai/trusted-partner.example"]
});
```

An agent executes a discovered `RegisteredTool` by passing the tool dictionary and input arguments along to `document.modelContext.executeTool()`. The browser securely mediates the execution, ensuring the [`exposedTo`](https:-ai/-ai/webmachinelearning.github.io-ai/webmcp-ai/#dom-modelcontextregistertooloptions-exposedto) and [`fromOrigins`](https:-ai/-ai/webmachinelearning.github.io-ai/webmcp-ai/#dom-modelcontextgettooloptions-fromorigins) agree, and the tool runs in the tool owner's execution context:
...
-ai/-ai/ If the user cancels the interaction:
stopButton.addEventListener('click', e => controller.abort());
```

The tool's [execution callback](https:-ai/-ai/webmachinelearning.github.io-ai/webmcp-ai/#callbackdef-toolexecutecallback) receives this signal via its [`options.signal`](https:-ai/-ai/webmachinelearning.github.io-ai/webmcp-ai/#dom-modelcontextexecutetooloptions-signal) parameter, allowing it to abort underlying network requests or asynchronous tasks cleanly.
...
- MCP was built primarily for server-to-client and stdio-ai/SSE process communication. It lacks native web concepts like origins, standard browser permissions, DOM integration, and tab-level lifecycle management.
...
- **WebMCP (MCP-B)**: An open-source project (see [MCP-B](https:-ai/-ai/mcp-b.ai-ai/)) implementing browser tab and extension transports for local in-page communication.
...
For our current considerations, refer to the [Security and Privacy Considerations](https:-ai/-ai/webmachinelearning.github.io-ai/webmcp-ai/#security-privacy) section of the specification.
...
- **Multimodal input-ai/output**: AI agents are increasingly multimodal, and we should consider how tools can consume binary media as inputs and how to return them as outputs (e.g., audio, streams, media blobs, etc.). See [Issue #41](https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/41), [Issue #86](https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/86), and [Issue #81](https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/81), and [Prompt API: Multimodal inputs](https:-ai/-ai/github.com-ai/webmachinelearning-ai/prompt-api#multimodal-inputs).

- **Cross-document tool response**: How should WebMCP handle tool responses when a tool (a form submission, for example) causes the page to navigate to another document? See [Issue #135](https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/135).

- **Built-in agent exposure by default**: <p id=built-in-agent-default-exposure>The [`exposedTo`](https:-ai/-ai/webmachinelearning.github.io-ai/webmcp-ai/#dom-modelcontextregistertooloptions-exposedto) array only takes origins, but we're considering introducing a new keyword like `native-agent`, letting authors control a tool's exposure to a built-in agent. The running idea is that by default in the top-level document, a missing `exposedTo` array would expose tools to the built-in agent, and in iframes, a missing `exposedTo` array would not expose tools to the built-in agent<-ai/p>

- **Transferable-ai/streamable tool inputs and outputs**: AI models inherently support streaming data. WebMCP should consider enabling streaming tool inputs and outputs (such as chunked generation or large data transfers) without blocking on a massive copy. See [Issue #82](https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/82). See also [MCP discussion](https:-ai/-ai/github.com-ai/modelcontextprotocol-ai/modelcontextprotocol-ai/discussions-ai/263) and [MCP Apps streaming tool inputs](https:-ai/-ai/github.com-ai/modelcontextprotocol-ai/ext-apps-ai/blob-ai/main-ai/specification-ai/draft-ai/apps.mdx#notifications-host--view).

- **Input and output schema validation**: Investigating native validation of tool inputs and outputs against declared JSON schemas before invoking the page's JS execution callback, or letting the output reach the model. See [Issue #92](https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/92).

- **Skills Integration**: Determining if the author should expose a higher-level "skill" to help the agent coordinate multiple related tools to fulfill a user journey. See [Issue #161](https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/161).

- **Output schema**: Supporting structured `outputSchema` contracts (complementing `inputSchema`) to help LLMs reliably reason about the return values of tools. See [Issue #9](https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/9).

- **User prompting and elicitation**: Exploring a way for a tool to prompt the user for confirmation when tools require explicit user authorization. This could be done by delegating to the agent and its harness, or by invoking native browser permission dialogue outside of the agent loop. See [Issue #165](https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/165) and [Issue #50](https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp-ai/issues-ai/50) for discussion about the `ModelContextClient` interface.

- **Tool progress reporting**: For long-running tasks (e.g., batch processing or generating content), the agent may want a way to track a tool's progress. We are exploring how this intersects with the established [MCP Progress](https:-ai/-ai/modelcontextprotocol.io-ai/specification-ai/2025-11-25-ai/basic-ai/utilities-ai/progress) specification.

- **Service workers integration**: Extending WebMCP to background Service Workers to allow agents to discover and invoke tools on sites the user doesn't currently have open. This is detailed in the supplementary [Service Workers Explainer](.-ai/docs-ai/service-workers.md), which proposes background discovery mechanisms, session identification, and JIT worker installation.
...
>
> Brandon Walderman <code>&lt;brwalder@microsoft.com&gt;<-ai/code><br>
> Leo Lee <code>&lt;leo.lee@microsoft.com&gt;<-ai/code><br>
> Andrew Nolan <code>&lt;annolan@microsoft.com&gt;<-ai/code><br>
> David Bokan <code>&lt;bokan@google.com&gt;<-ai/code><br>
> Khushal Sagar <code>&lt;khushalsagar@google.com&gt;<-ai/code><br>
> Hannah Van Opstal <code>&lt;hvanopstal@google.com&gt;<-ai/code>

Since then, the specification draft has evolved significantly, primarily driven by [Dominic Farolino](https:-ai/-ai/github.com-ai/domfarolino).

Many thanks to [Alex Nahas](https:-ai/-ai/github.com-ai/MiguelsPizza) and [Jason McGhee](https:-ai/-ai/github.com-ai/jasonjmcghee-ai/) for sharing their valuable [implementation](https:-ai/-ai/github.com-ai/MiguelsPizza-ai/WebMCP) [experience](https:-ai/-ai/github.com-ai/jasonjmcghee-ai/WebMCP).
