...
This is a supplemental explainer for the main WebMCP explainer at https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp.

## The Problem

The initial explainer for [WebMCP](https:-ai/-ai/github.com-ai/webmachinelearning-ai/webmcp) covers how web pages expose context to AI agents that are already browsing a page. WebMCP's design makes it possible for in-browser AI integrations such as sidebar assistants to access context on currently opened browser tabs, and to operate the pages within those tabs.
...
_The agent recognizes that `http:-ai/-ai/todoapp.example` has registered tools via its service worker. The service worker has exposed the following tool:_
...
_The agent recognizes that `http:-ai/-ai/freshmart.example` has registered tools via its service worker. The service worker has exposed several tools:_
...
2.	Since Mike doesn't have a payment method saved, opening a new browser window to `http:-ai/-ai/freshmart.example-ai/checkout`
...
The experimental [Payment Handler API](https:-ai/-ai/developer.mozilla.org-ai/en-US-ai/docs-ai/Web-ai/API-ai/Payment_Handler_API) has already introduced the ["serviceworker"](https:-ai/-ai/developer.mozilla.org-ai/en-US-ai/docs-ai/Web-ai/Progressive_web_apps-ai/Manifest-ai/Reference-ai/serviceworker) app manifest field which is used for Just-In-Time (JIT) registering of service workers to handle payment requests. The app manifest includes the URL for a service worker script so that the browser can directly fetch it and then install the service worker without requiring the user to navigate to a page first:
...
participant F as Service Worker: example.com-ai/worker.js
...
In the initial WebMCP API proposal, tools are associated with a web page in a specific tab-ai/window, and there is an implicit 1:1 mapping between the page, and an AI assistant operating that page. In other words, tool calls are handled only by one page and are sent from only one agent.
...
A browser process may have one or more tabs opened to the same site and may also have a service worker installed for that site. Each of these may have WebMCP tools (some of which are the same), but each tab-ai/worker can be thought of as separate MCP servers. Which of these server(s) receive tool calls will depend on how-ai/whether any MCP clients have connected to them. There are a few possibilities:
  ...
  tools: [
    {
      ...
      async execute(params, clientInfo) {
        -ai/-ai/ fetch shopping cart for this session.
...
A potential starting point to make WebMCP tool use in LLM-backed agents secure is to limit an agent session-ai/conversation to a single origin-ai/scope. That is, as soon as an LLM-backed agent accesses a service worker's WebMCP tools, it should be limited to only accessing tools from that worker's scope. Although not a complete mitigation for points 1 and 2, this helps limit the scope of user data the conversation has access to. Disabling web search and other external communication methods for the remainder of the conversation would help mitigate data exfiltration, addressing point 3 above.
